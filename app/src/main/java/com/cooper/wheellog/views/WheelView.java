...
public class WheelView extends View {
    ...
    private boolean isSpeedSpeechEnabled;
    private int lastSpeechSpeed;
    private long lastSpeechSpeedMs;
    private long sameSpeedSpeechSecondsInterval;
    private long speedSpeechStartFromSpeed;
        ...
        textToSpeech = new TextToSpeech(mContext, new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    //textToSpeech.setPitch(10);
//                String textToSay = "Hello world, this is a test message!";
                    //              textToSpeech.speak(textToSay, TextToSpeech.QUEUE_ADD, null);
                }
            }
        });
    ...
    public void setSpeed(int speed) {
        ...
        //Log.i("WHEELLOGTEST",targetSpeed + "" );
    ...
    public void setSpeedSpeechEnabled(boolean enabled) {
        isSpeedSpeechEnabled = enabled;
    }
    public void setSpeedSpeechSameSpeedInterval(long seconds){
        sameSpeedSpeechSecondsInterval = seconds;
    }

    public void setSpeechStartFromSpeed(long speedSpeechFrom){
        speedSpeechStartFromSpeed = speedSpeechFrom;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Log.i("WHEELLOGTEST2", Math.round(speed/10.0) + "");

        if (isSpeedSpeechEnabled) {
            if ((int) Math.round(speed / 10.0) >=  speedSpeechStartFromSpeed) {

                if (lastSpeechSpeed != (int) Math.round(speed / 10.0)) {
                    lastSpeechSpeed = (int) Math.round(speed / 10.0);
                    String textToSay = lastSpeechSpeed + "";
                    textToSpeech.speak(textToSay, TextToSpeech.QUEUE_ADD, null);
                    lastSpeechSpeedMs = System.currentTimeMillis();
                } else {
                    if (sameSpeedSpeechSecondsInterval != 0) {
                        long curMs = System.currentTimeMillis();
                        if (curMs - lastSpeechSpeedMs > sameSpeedSpeechSecondsInterval * 1000) {
                            String textToSay = lastSpeechSpeed + "";
                            textToSpeech.speak(textToSay, TextToSpeech.QUEUE_ADD, null);
                            lastSpeechSpeedMs = System.currentTimeMillis();
                        }
